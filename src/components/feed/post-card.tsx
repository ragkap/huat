...
function ShareButton({ postId }: { postId: string; postContent: string; authorName: string; tickers?: string[] }) {
  ...
  return (
    <button
      onClick={handleShare}
      className="relative flex items-center justify-center w-8 h-8 rounded-full text-[#555555] hover:text-[#9CA3AF] transition-colors ml-auto"
    >
      {copied ? <Check className="w-[16px] h-[16px] text-[#22C55E]" /> : <Upload className="w-[16px] h-[16px]" />}
    </button>
  );
}
